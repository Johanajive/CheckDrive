...
export class InitMigration1763590676077 implements MigrationInterface {
    name = 'InitMigration1763590676077'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`appointments\` (\`id_appointment\` int NOT NULL AUTO_INCREMENT, \`date\` date NOT NULL, \`time\` time NOT NULL, \`status\` varchar(20) NOT NULL DEFAULT 'Active', \`userId\` int NULL, \`vehicleId\` int NULL, \`centerId\` int NULL, PRIMARY KEY (\`id_appointment\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`appointments\` ADD CONSTRAINT \`FK_550d0e0c440b5a8a1701aa8efe7\` FOREIGN KEY (\`vehicleId\`) REFERENCES \`vehicle_entity\`(\`id_vehicle\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`appointments\` ADD CONSTRAINT \`FK_030bde20415336d2278dad3e149\` FOREIGN KEY (\`centerId\`) REFERENCES \`inspection_center_entity\`(\`id_center\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`appointments\` DROP FOREIGN KEY \`FK_030bde20415336d2278dad3e149\``);
        await queryRunner.query(`ALTER TABLE \`appointments\` DROP FOREIGN KEY \`FK_550d0e0c440b5a8a1701aa8efe7\``);
